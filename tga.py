...
class TGAFooter(object):
    ...
    def __init__(self):
        ...
        self.__signature = bytes(
            bytearray("TRUEVISION-XFILE".encode("ascii"))
        )  # 16 bytes
...
MATRIX_TYPE = {"BW": "B", "RGB": "BBB", "RGBA": "BBBB"}
...
class PixelMatrix(object):
        ...
        if data is not None:
            if isinstance(data, list):
                ...
                if elm_size == 1:
                    ...
                    raise Exception(
                        "Data not compatible with BW, RGB or RGBA format, tuple has lenght {}".format(
                            elm_size
                        )
                    )
    ...
    def __buffer_from_data(self, data):
        if self.__type == MATRIX_TYPE["BW"]:
            self.__buffer.write(
                pack(
                    str("<") + str(self.__type) * self.__width * self.__height,
...
class Image(object):
    ...
    def check(data):
        ...
        for row in data:
            ...
            for pixel in row:
                ...
                elif type(pixel) != int and not re.match(
                    ".*numpy\.[u]?int(8|16|32|64)", str(type(pixel))
                ):
                    raise ImageError(
    ...
    def load(self, file_name):
        ...
        with open(file_name, "rb") as image_file:
            ...
            if (
                signature == "TRUEVISION-XFILE".encode("ascii")
                and dot == ".".encode("ascii")
                and zero == 0
            ):
                self.__new_TGA_format = True
            else:
                ...
                for row in range(self._header.image_height):
                    ...
                    for col in range(self._header.image_width):
                        if self._header.image_type == 3:
                            tmp[row].append(dec_byte(image_file.read(1)))
                            ...
                            if self._header.pixel_depht == 16:
                                ...
                                c_b, c_g, c_r = multiple_dec_byte(image_file, 3)
                                tmp[row].append((c_r, c_g, c_b))
                            elif self._header.pixel_depht == 32:
                                c_b, c_g, c_r, alpha = multiple_dec_byte(image_file, 4)
                                tmp[row].append((c_r, c_g, c_b, alpha))
                        else:
                            raise ImageError(
                ...
                while pixel_count != tot_pixels:
                    ...
                    if RLE:
                        ...
                        if self._header.image_type == 11:
                            ...
                            if self._header.pixel_depht == 16:
                                pixel = get_rgb_from_16(dec_byte(image_file.read(2), 2))
                            elif self._header.pixel_depht == 24:
                                c_b, c_g, c_r = multiple_dec_byte(image_file, 3)
                                pixel = (c_r, c_g, c_b)
                            elif self._header.pixel_depht == 32:
                        ...
                        for num in range(count):
                            if self._header.image_type == 11:
                                tmp[-1].append(dec_byte(image_file.read(1)))
                            elif self._header.image_type == 10:
                                if self._header.pixel_depht == 16:
                                    tmp[-1].append(
                                        get_rgb_from_16(dec_byte(image_file.read(2), 2))
                                    ...
                                    c_b, c_g, c_r = multiple_dec_byte(image_file, 3, 1)
                                    tmp[-1].append((c_r, c_g, c_b))
                                elif self._header.pixel_depht == 32:
                                    c_b, c_g, c_r, alpha = multiple_dec_byte(
                                        image_file, 4, 1
                                    )
                                    tmp[-1].append((c_r, c_g, c_b, alpha))
                            else:
                                raise ImageError(
                                    "type num '{0}'' is not supported".format(
                                        self._header.image_type
    ...
    def save(
        self, file_name, original_format=False, force_16_bit=False, compress=False
    ):
                    ...
                    for repetition_count, pixel_value in self._encode(row):
                        ...
                        if repetition_count > 127:
                            if self._header.image_type == 11:
                                ...
                                if self._header.pixel_depht == 16:
                                    image_file.write(gen_pixel_rgb_16(*pixel_value))
                                elif self._header.pixel_depht == 24:
                                    image_file.write(gen_pixel_rgba(*pixel_value))
                                elif self._header.pixel_depht == 32:
                                    image_file.write(gen_pixel_rgba(*pixel_value))
                        else:
...
# where is position of the origin in cartesian coordinates
class CoordinateType(IntEnum):
    LB = 1  # the origin at the left bottom corner of the image
